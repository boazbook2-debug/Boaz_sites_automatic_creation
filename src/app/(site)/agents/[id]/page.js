...
export default async function AgentProfilePage({ params }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-6 sm:py-10 lg:px-10">
      ...
      <div className="flex flex-col items-center gap-4 rounded-2xl bg-[var(--color-surface)] p-5 text-center shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] sm:flex-row sm:gap-6 sm:rounded-[2rem] sm:p-6 sm:text-right">
        <SampleImage
          src={agent.photo}
          alt={agent.name}
        ...
        />
        <div>
          <h1 className="text-2xl font-serif font-bold tracking-tight sm:text-3xl">{agent.name}</h1>
      ...
      {agent.bio && (
        <div className="mt-5 rounded-2xl bg-[var(--color-surface)] p-5 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] sm:mt-6 sm:rounded-[2rem] sm:p-6">
          <QuoteIcon className="h-6 w-6 text-[var(--color-accent2)] sm:h-8 sm:w-8" />
          <p className="mt-3 text-sm leading-relaxed text-[var(--color-main)]/80 sm:text-lg">
      ...
      {agentProperties.length > 0 && (
        <div className="mt-6 sm:mt-10">
          <h2 className="mb-4 text-lg font-serif font-bold tracking-tight sm:mb-6 sm:text-2xl">הנכסים שלי</h2>
          <PropertyGrid properties={agentProperties} />
        </div>
      )}

      <div className="mt-6 sm:mt-10">
        <LeadForm title={`השאירו פרטים ל${agent.name}`} toWhatsapp={agent.whatsapp} />
      </div>
    </div>
  );
}
